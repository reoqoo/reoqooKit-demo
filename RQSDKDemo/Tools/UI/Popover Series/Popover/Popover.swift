...
open class Popover: UIView {
    ...
    // custom property
    open var arrowSize: CGSize = CGSize(
        width: 16.0,
        height: 10.0
    )
    ...
    open var blackOverlayColor: UIColor = UIColor(
        white: 0.0,
        alpha: 0.2
    )
    ...
    open func showAsDialog(_ contentView: UIView) {
        guard let rootView = UIApplication.mainWindow else { return}
    ...
    open func show(_ contentView: UIView, fromView: UIView, inView: UIView) {
        ...
        if self.popoverType == .verticalAuto {
            if let point = fromView.superview?.convert(fromView.frame.origin, to: nil), point.y + fromView.frame.height + self.arrowSize.height + contentView.frame.height > inView.frame.height {
                self.popoverType = .up
            } else {
        ...
        if self.popoverType == .horizontalAuto {
            if let point = fromView.superview?.convert(fromView.frame.origin, to: nil), point.x + fromView.frame.width + self.arrowSize.width + contentView.frame.width > inView.frame.width {
                self.popoverType = .left
            } else {
                self.popoverType = .right
            }
        }
        ...
        case .left:
            point = inView.convert(CGPoint(x: fromView.frame.origin.x - sideOffset, y: fromView.frame.origin.y + 0.5 * fromView.frame.height), from: fromView.superview)
        case .right, .horizontalAuto:
            point = inView.convert(CGPoint(x: fromView.frame.origin.x + fromView.frame.size.width + sideOffset, y: fromView.frame.origin.y + 0.5 * fromView.frame.height), from: fromView.superview)
        }
    ...
    @objc open func dismiss() {
        if self.superview != nil {
            ...
            UIView.animate(withDuration: self.animationOut,
                           delay: 0,
    ...
    override open func draw(
        ...
        let arrowPoint = self.containerView.convert(
            self.arrowShowPoint,
            to: self
        )
        switch self.popoverType {
        case .up:
            ...
            arrow.addArc(
                ...
                startAngle: self.radians(
                    90
                ),
                endAngle: self.radians(
                    180
                ),
                clockwise: true
            )
        ...
        case .right, .horizontalAuto:
            ...
            arrow.addLine(
                to: CGPoint(
                    x: self.bounds.width + arrowPoint.x,
                    y: self.cornerRadius
                )
            )
            arrow.addArc(
...
private extension Popover {
    ...
    func create() {
        ...
        case .up:
            ...
            anchorPoint = CGPoint(
                x: arrowPoint.x / frame.size.width,
                y: 1
            )
        case .down, .verticalAuto:
        ...
        let x = self.layer.position.x + (
            anchorPoint.x - lastAnchor.x
        ) * self.layer.bounds.size.width
        let y = self.layer.position.y + (
            anchorPoint.y - lastAnchor.y
        ) * self.layer.bounds.size.height
        self.layer.position = CGPoint(
            x: x,
            y: y
        )
    ...
    func createHighlightLayer(
        ...
        let highlightRect = inView.convert(
            fromView.frame,
        ...
        self.blackOverlay.layer.addSublayer(
            fillLayer
        )
    }
    ...
    var isCloseToCornerRightArrow: Bool {
        return self.arrowShowPoint.x > (
            self.frame.origin.x + self.bounds.width
        ) - arrowSize.width/2 - cornerRadius
    }
